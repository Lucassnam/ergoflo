  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Raw <img>: these are remote Unsplash URLs, so next/image would
              need images.remotePatterns and buy us nothing at this size. */}
          <img
            src={src}
            alt=""
            aria-hidden="true"
        ...
        {/* Back */}
        <div
          className="absolute inset-0 flex h-full w-full flex-col items-center justify-center overflow-hidden rounded-xl border border-neutral-200 bg-white p-4 shadow-lg"
          style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
        >
          <div className="text-center">
            <p className="mb-1 font-mono text-[8px] tracking-widest text-neutral-400 uppercase">
              Still
            </p>
            <p className="headline text-sm text-neutral-900">Dry</p>
          </div>
        </div>
      </motion.div>
    </motion.div>
...
/**
 * Deterministic 0..1 noise keyed off the card index. The upstream component
 * used Math.random() during render, which makes the server and client
 * disagree on the scatter and produces a hydration mismatch.
 */
const indexNoise = (i: number, salt: number) => {
...
export default function ScrollMorphHero() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Hand the wheel back to the page at either bound, otherwise the hero
      // swallows every scroll event and the reader can never leave it.
  ...
  // Sweeps the arc once it has formed.
  ...
  const scatterPositions = useMemo(
    () =>
      IMAGES.map((_, i) => ({
        x: (indexNoise(i, 1) - 0.5) * 1500,
        y: (indexNoise(i, 2) - 0.5) * 1000,
        rotation: (indexNoise(i, 3) - 0.5) * 180,
        scale: 0.6,
        opacity: 0,
      })),
    []
  ...
  return (
    ...
    >
      <div className="flex h-full w-full flex-col items-center justify-center">
        {/* Opening headline — fades out as the arc forms. Sits above the
            cards and stays narrow so the circle never runs through it. The
            radial wash keeps it legible when a card drifts behind. */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute top-1/2 left-1/2 z-10 h-[420px] w-[620px] max-w-full -translate-x-1/2 -translate-y-1/2"
          style={{
            background:
              "radial-gradient(ellipse at center, rgba(255,255,255,0.94) 0%, rgba(255,255,255,0.75) 45%, transparent 72%)",
          }}
        />
        <div className="pointer-events-none absolute top-1/2 z-20 flex max-w-[16ch] -translate-y-1/2 flex-col items-center justify-center text-center">
          <motion.h1
            ...
            className="headline px-6 text-4xl text-neutral-900 md:text-6xl"
          >
            Never sweat
            <br />
            again.
          ...
          <motion.p
            ...
            className="mt-4 font-mono text-[11px] font-bold tracking-[0.2em] text-neutral-500 uppercase"
          >
            Scroll to explore
          </motion.p>
        </div>
        ...
        <div className="relative flex h-full w-full items-center justify-center">
          {IMAGES.slice(0, TOTAL_IMAGES).map((src, i) => {
            ...
            if (introPhase === "scatter") {
              ...
              // Sits the arc well below the copy block; at 0.25 the topmost
              // cards climbed into the headline.
              ...
              // Stop short of a full sweep so the last card stays on screen.
